...
func (l *Lexer) NextToken() token.Token {
	...
	case '=':
		if l.peekChar() == '=' {
			tok = token.Token{Type: token.EQ, Literal: l.readDouble()}
		} else {
	...
	case '!':
		if l.peekChar() == '=' {
			tok = token.Token{Type: token.NOT_EQ, Literal: l.readDouble()}
		} else {
	...
	case '\\':
		tok = newToken(token.BKSLASH, l.ch)
	case '*':
...
func (l *Lexer) readDouble() string {
	position := l.position
	l.readChar()
	return l.input[position:(l.position + 1)]
}

// Advance the char position until non-whitespace is found
